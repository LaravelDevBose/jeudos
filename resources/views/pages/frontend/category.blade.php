...
@section('content')
    ...
    @if(!empty($subCategories) && count($subCategories) > 0)
        @foreach($subCategories as $subCategory)
            <div class="row">
                <div class="col-md-12 align-items-center">
                    <h1 class="float-left text-bold font-size-24">{{$subCategory->name}}</h1>
                </div>
            </div>
            <div class="row">
                @foreach($subCategory->subCategoryUser as $influencerUser)
                    <div class="col-xl-2 col-lg-3 col-sm-5 col-xs-8 col-10">
                        <a href="{{route('influencer-profile',$influencerUser->slug)}}">
                            <div class="card bg-white shadow influencer" style="border-radius: 10px; height: 210px; overflow: hidden">
                                @if(!empty($influencerUser->profile_image_url) && !is_null($influencerUser->profile_image_url))
                                    <img src="{{asset($influencerUser->profile_image_url)}}" alt="{{$influencerUser->name}}" class="card-img" style="border-radius: 10px; height: 100%; width: 100%"/>
                                @else
                                    <img src="{{asset('backend/images/user.png')}}" alt="{{$influencerUser->name}}" class="card-img" style="border-radius: 10px; height: 100%; width: 100%"/>
                                @endif
                                <div class="card-img-overlay">
                                    <h4 class="text-white">{{$influencerUser->name}}</h4>
                                    <h6 class="text-white">{{$influencerUser->title}}</h6>
                                    <h4 class="text-white mt-100 pr-0"><small>&#36;{{number_format($influencerUser->rate)}}</small></h4>
                                </div>
                            </div>
                        </a>
                    </div>
