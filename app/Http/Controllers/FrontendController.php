...
class FrontendController extends Controller
{
    ...
    public function category($id)
    {
        ...
        $subCategories = SubCategory::where('category_id', $id)->with('subCategoryUser')->get();
    ...
    public function influencer($influencer)
    {
        ...
        $influencer = User::where('slug', $influencer)->first();
        ...
        $offSchedules = InfluencerSchedule::select('start_date as from', 'end_date as to')->where('user_id', $influencer->id)->where('status', 1)->get();
        return view('pages.frontend.influencer', get_defined_vars());
    }

    public function influencer_schedule_check(Request $request)
    {
        $offSchedule = InfluencerSchedule::where('user_id', $request->id)->where('status', 1)
                        ->where('start_date', '<=', $request->date_time)
                        ->where('end_date', '>=', $request->date_time)->first();
        if(!empty($offSchedule)){

            return response()->json([
                'status'=>1,
                'message'=>'Not Available. Choose A Schedule After '.date('d M Y G:i A', strtotime($offSchedule->end_date)),
            ]);
        }else {
            $booking = Booking::where('influencer_id', $request->id)->where('status', 1)
                ->where('date', '<=', $request->date_time)
                ->where('end_date', '>=', $request->date_time)->first();
            if(!empty($booking)){
                return response()->json([
                    'status'=>1,
                    'message'=>'Not Available. Choose A Schedule After '.date('d M Y G:i A', strtotime($booking->end_date)),
                ]);
            }
            return response()->json([
                'status'=>0
    ...
    public function book(Request $request)
    {
        ...
        $response = $this->validatorResponseHandler($validator);
        if ($response !== true) return $response;
        $offSchedule = InfluencerSchedule::where('user_id', $request->influencer_id)->where('status', 1)
            ->where('start_date', '<=', $request->date)
            ->where('end_date', '>=', $request->date)->first();
        if(!empty($offSchedule)){
            $message='Not Available. Choose A Schedule After '.date('d M Y G:i A', strtotime($offSchedule->end_date));
            return $this->errorResponseHandler($message);
        }
        ...
        $booking = Booking::create([
            ...
            'end_date'=> Carbon::parse($request->date)->addMinutes($request->duration),
        ]);
